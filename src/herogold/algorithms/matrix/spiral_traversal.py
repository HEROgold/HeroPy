...
def spiral_traversal(matrix: list[list[int]]) -> list[int]:
    ...
    while row_begin <= row_end and col_begin <= col_end:
        result.extend(matrix[row_begin][i] for i in range(col_begin, col_end + 1))
        row_begin += 1

        result.extend(matrix[i][col_end] for i in range(row_begin, row_end + 1))
        col_end -= 1

        if row_begin <= row_end:
            result.extend(matrix[row_end][i] for i in range(col_end, col_begin - 1, -1))
        row_end -= 1

        if col_begin <= col_end:
            result.extend(matrix[i][col_begin] for i in range(row_end, row_begin - 1, -1))
        col_begin += 1
