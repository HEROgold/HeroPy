...
def rotate_v1[T](array: list[T], k: int) -> list[T]:
    """Rotate array to the right by k steps using repeated single shifts.

    Args:
        array: List of elements to rotate.
...
def rotate_v2[T](array: list[T], k: int) -> list[T]:
    ...
    def _reverse(arr: list[T], left: int, right: int) -> None:
        while left < right:
...
def rotate_v3[T](array: list[T] | None, k: int) -> list[T] | None:
    """Rotate array to the right by k steps using slicing.

    Args:
        array: List of elements to rotate, or None.
        k: Number of positions to rotate right.
