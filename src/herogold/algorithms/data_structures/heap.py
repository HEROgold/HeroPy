...
class AbstractHeap(Generic[T], metaclass=ABCMeta):
...
class BinaryHeap(AbstractHeap[T]):
    ...
    def perc_up(self, index: int) -> None:
        ...
        """
        while index // 2 > 0:
            if self.heap[index] < self.heap[index // 2]:  # type: ignore[operator]
    ...
    def min_child(self, index: int) -> int:
        ...
        if 2 * index + 1 > self.current_size:
            return 2 * index
        if self.heap[2 * index] > self.heap[2 * index + 1]:  # type: ignore[operator]
            return 2 * index + 1
        return 2 * index
    ...
    def perc_down(self, index: int) -> None:
        ...
        while 2 * index < self.current_size:
            ...
            if self.heap[smaller_child] < self.heap[index]:  # type: ignore[operator]
    ...
    def remove_min(self) -> T:
        ...
        """
        ret = self.heap[1]  # type: ignore[return-value]
