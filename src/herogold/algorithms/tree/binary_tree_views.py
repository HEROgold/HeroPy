...
def left_view[T](root: TreeNode[T] | None) -> list[T]:
...
def right_view[T](root: TreeNode[T] | None) -> list[T]:
...
def top_view[T](root: TreeNode[T] | None) -> list[T]:
    ...
    hd_map: dict[int, T] = {}
    queue: deque[tuple[TreeNode[T], int]] = deque([(root, 0)])
    while queue:
...
def bottom_view[T](root: TreeNode[T] | None) -> list[T]:
